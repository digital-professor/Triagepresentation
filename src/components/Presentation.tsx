    ...
    <div className="bg-white rounded-xl p-6 md:p-12 shadow-xl min-h-full">
      ...
      <div className="flex flex-col items-center gap-4 mb-8">
        {stages.map((stage, idx) => {
          const Icon = stage.icon as any; const widthPct = stage.pct;
...
function InvestmentSlide() {
  ...
  return (
    <div className="bg-white rounded-xl p-6 md:p-12 shadow-xl min-h-full">
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div>
          <h2 className="text-slate-900 mb-4">Future Applications (Scaling Out)</h2>
          <div className="space-y-4">
            {futureApplications.map((item, index) => {
              const Icon = item.icon as any;
              return (
                <div key={index} className="bg-slate-50 border-l-4 border-slate-400 rounded-lg p-4 flex items-start gap-4">
                  <Icon className="h-6 w-6 text-slate-600 flex-shrink-0 mt-1" />
                  <div><h3 className="text-slate-900">{item.title}</h3><p className="text-slate-700 text-sm">{item.description}</p></div>
                </div>
              );
            })}
          </div>
        ...
        <div>
          <h2 className="text-slate-900 mb-4">Future Automation (Scaling Up)</h2>
          <div className="space-y-4">
            {futureAutomations.map((item, index) => {
              const Icon = item.icon as any;
              return (
                <div key={index} className="bg-slate-50 border-l-4 border-slate-400 rounded-lg p-4 flex items-start gap-4">
                  <Icon className="h-6 w-6 text-slate-600 flex-shrink-0 mt-1" />
                  <div>
                    ...
                    <p
                      ...
                      dangerouslySetInnerHTML={{ __html: item.description.replace(/\*(.*?)\*/g, "<i class='italic'>$1</i>") }}
                    />
                  </div>
                </div>
              );
...
function RecommendationSlide() {
  const benefits = [
    { icon: TrendingUp, title: "Maximise Efficiency", description: "Direct advisor time to high-value cases" },
    { icon: Database, title: "Data-Driven Decisions", description: "Enable evidence-based interventions" },
    ...
    <div className="bg-white rounded-xl p-6 md:p-12 shadow-xl min-h-full">
      ...
      <div className="bg-gradient-to-br from-green-500 to-green-700 rounded-2xl p-6 md:p-12 text-white mb-8 shadow-2xl">
        <div className="flex flex-col md:flex-row items-start md:items-center gap-4 md:gap-6 mb-6">
          <div className="bg-white/20 rounded-full p-4 md:p-6"><CheckCircle className="h-12 w-12 md:h-16 md:w-16" /></div>
        ...
        <p className="text-green-50 leading-relaxed">
          Based on the pilot's exceptional accuracy, measurable ROI in saved time, and critical strategic insights, this proactive triage model should be adopted as our <span className="font-semibold">standard operating procedure</span> for managing at-risk student cohorts within Student Advice.
        </p>
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-6">
        {benefits.map((benefit, index) => {
          ...
          return (
            <div key={index} className="bg-gradient-to-br from-slate-50 to-slate-100 border-2 border-slate-200 rounded-xl p-6 text-center hover:shadow-lg transition-shadow">
              <div className="bg-green-100 rounded-full p-4 inline-flex mb-4"><Icon className="h-8 w-8 md:h-10 md:w-10 text-green-600" /></div>
...
function QASlide() {
  const stats = [
    { label: "Response Rate", value: "~63% (38/60)" },
    { label: "Efficiency Gain", value: "~79%" },
    { label: "Advisor Hours Saved", value: "16.5" },
      ...
      <div className="text-center">
        <div className="inline-block bg-gradient-to-r from-blue-600 to-purple-600 rounded-full p-1">
          <div className="bg-white rounded-full px-6 md:px-12 py-4 md:py-6">
            <p className="text-slate-900">Thank you for your time and consideration</p>
          </div>
        </div>
      </div>
    </div>
...
// --- Main App Component (Navigator renders only ONE set of buttons) ---
export default function Presentation() {
  ...
  // indicator block reused for both layouts
  const Indicator = (
